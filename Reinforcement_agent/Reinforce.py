...
class Agent():
    def __init__(self, env, shapes, rewards, dropout=0.6):
        self.env = env
    ...
    #         FOR FUTURE ========================================================================
            ...
            self.activations['Z{}'.format(i+2)] = tf.add(tf.matmul(Ai, Wi), bi)
    ...
    def initialize(self):
        ...
        return network_state, network_action, network_reward, keep_prob, loss

    def train(self, epsilon = 0.6, BATCH_SIZE=32, GAMMA= 0.8, MAX_LEN_REPLAY_MEMORY = 30000,
        ...
        # network_state, network_action, network_reward, action_onehot, keep_prob = self.create_placeholders()
        network_state, network_action, network_reward, keep_prob, loss = self.initialize()
        ...
        with tf.Session() as sess:      
            ...
            for i_epoch in range(FRAMES_TO_PLAY):
                if epsilon_decay:
                    epsilon = self.decay(i_epoch, FRAMES_TO_PLAY)
                ...
                if len(replay_memory) > MIN_FRAMES_FOR_LEARNING:
                    ...
                    if i_epoch%10000==1 and persistent:
    ...
    def save(self, sess, FRAMES_TO_PLAY):
        ...
        save_path = saver.save(sess, './models/model{}_{}'.format(len(self.architecture), 69696))
    ...
    def run(self, print_time=True, debug=False):
        tf.reset_default_graph()
        init = tf.global_variables_initializer()
        with tf.Session() as sess:
